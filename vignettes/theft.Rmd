...
The function `plot_correlation_matrix` automates the calculation of correlations between time-series values of each unique time series, the hierarchical clustering of rows and columns of these correlations, and the production of a final heatmap-style graphic. You can switch the default Pearson correlation to a Spearman rank correlation by changing the `cor_method` argument to `spearman`.
...
2. `FeatureFeatureCorrelations` -- a `ggplot` containing the pairwise correlations between top performing features represented as a heatmap
3. `DiscriminationPlots` -- a `ggplot` containing a matrix of violin plots showing class discrimination by feature
...
The three core functions that comprise `compute_top_features` are all available individually for your own usage as `fit_feature_classifier`, `plot_correlation_matrix`, and `plot_feature_discrimination`.
