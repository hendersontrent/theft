...
test_that("catch22 feature calculation", {
  ...
  expect_equivalent(sub_catch22$values,
...
test_that("feasts feature calculation", {
  ...
  expect_equivalent(sub_feasts$values,
...
test_that("tsfeatures feature calculation", {
  
  features_tsfeatures <- calculate_features(data = tmp, 
                                            id_var = "id", 
                                            time_var = "timepoint", 
                                            values_var = "values", 
                                            group_var = "process", 
  ...
  expect_equal(62 * length(unique(tmp$id)), 
               nrow(features_tsfeatures))
  
  sub_tsfeatures <- features_tsfeatures[features_tsfeatures$id == unique(tmp$id)[1], ]
  
  expect_equivalent(sub_tsfeatures$values,
                    c(1, 0, 1, 0.072296444522295, 0.000131347086079623, -1.19486201662717, 
                      -0.457482210368376, -0.0599597956226268, 0.119892570623111, 1, 
                      0.00930417330152654, 0.0936461319341436, -0.510181541689284, 
                      0.368022825410556, -0.66891833455421, 0.618467095765786, 0.394495412844037, 
                      0.623853211009174, 0.02524146256414, 1, -0.337971798248685, 2.03446927525368, 
                      0.544642857142857, 1, 1, 1.74037030215935, 1.4134132742367, 2.07, 1.43, -0.25, 
                      0.0973451327433628, 0.162162162162162, 0.0896141376556983, 63, 4, 0.0895069312730215, 
                      0.0914180569579202, 0.0896141376556983, 0.0935525737527965, 0.000100049003591104, 
                      0.000100001511301969, 0.500045830129239, 0.200199908738181, 0.670158766355093, 10, 
                      0.985907267579202, 44, 1.47062301134725, 21, 0.461005758783119, 0.0288257547334132, 
                      0.452315378731658, 1.02800341843051, 0.0902236301387019, 0.24888709987701, -113.272689613385, 
                      0.394495412844037, 4, -0.25, 1, 1.74037030215935, 2.43))
})
