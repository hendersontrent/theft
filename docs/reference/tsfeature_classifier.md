...
- data:
  ...
  produced by `calculate_features` with an included `group` column as
  per
...
- classifier:
  ...
  with R's `predict` functionality. Please note that
  ...
  uses it is recommended. Defaults to `NULL` which means the following
  linear SVM is fit:
